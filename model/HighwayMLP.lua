...
function HighwayMLP.mlp(size, num_layers, bias, f)
  ...
  local f = f or nn.ReLU()  -- Why do we use ReLU as default? --> it is default activation for MLP
  local input = nn.Identity()()
  local inputs = {[1]=input}
  for i = 1, num_layers do
    output = f(nn.Linear(size, size)(inputs[i]))  -- Square matrix  (Where is bias for output? --> omitted)
