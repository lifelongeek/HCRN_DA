
-- This is multi-layered implementation

local GRU_uni_multi_diagRNN = {}
function GRU_uni_multi_diagRNN.gru(input_size, L_RNN, rnn_size, dropout,output_size)
  dropout = dropout or 0
  -- there are n+1 inputs (hiddens on each layer and x)
  local inputs = {}
  -- Ver1 : without speaker info
  -- table.insert(inputs, nn.Identity()()) -- x

  -- Ver2 : with speaker info
  table.insert(inputs, nn.Identity()()) -- embedVec
  table.insert(inputs, nn.Identity()()) -- speakerVec

  for L = 1,L_RNN do
    table.insert(inputs, nn.Identity()()) -- prev_h[L]
  end

  function new_input_sum(insize, xv, hv)
    local i2h = nn.Linear(insize, rnn_size)(xv)
    local h2h = nn.Linear(rnn_size, rnn_size)(hv)
    return nn.CAddTable()({i2h, h2h})
  end

  local x, input_size_L
  local outputs = {}
  for L = 1,L_RNN do
    -- Ver1 : Without speaker info
    -- local prev_h = inputs[L+1]
    -- Ver2 : With speaker info
    local prev_h = inputs[L+2]

    -- the input to this layer
    if L == 1 then
      --Ver1 : Without speaker info
      --[[
      x = inputs[1]
      input_size_L = input_size
      --]]

      --Ver2 : with speaker info
      embedVec = inputs[1]
      spkVec = inputs[2]      
      x = nn.JoinTable(1)({embedVec,spkVec})
      input_size_L = input_size+2
    else
      x = outputs[(L-1)]
      if dropout > 0 then x = nn.Dropout(dropout)(x) end -- apply dropout, if any
      input_size_L = rnn_size
    end
    -- GRU tick
    -- forward the update and reset gates
    local update_gate = nn.Sigmoid()(new_input_sum(input_size_L, x, prev_h))
    local reset_gate = nn.Sigmoid()(new_input_sum(input_size_L, x, prev_h))
    -- compute candidate hidden state
    local gated_hidden = nn.CMulTable()({reset_gate, prev_h})
    local p2 = nn.Linear(rnn_size, rnn_size)(gated_hidden)
    local p1 = nn.Linear(input_size_L, rnn_size)(x)
    local hidden_candidate = nn.Tanh()(nn.CAddTable()({p1,p2}))
    -- compute new interpolated hidden state, based on the update gate
    local zh = nn.CMulTable()({update_gate, hidden_candidate})
    local zhm1 = nn.CMulTable()({nn.AddConstant(1,false)(nn.MulConstant(-1,false)(update_gate)), prev_h})
    local next_h = nn.CAddTable()({zh, zhm1})

    table.insert(outputs, next_h)
  end

  -- set up the final output
  local top_h = outputs[#outputs]
  if dropout > 0 then top_h = nn.Dropout(dropout)(top_h) end
  fc = nn.Sequential()
  fc:add(nn.Linear(opt.H_dRNN, opt.H_fc))
  -- fc:add(nn.ReLU())
  fc:add(nn.LeakyReLU())

  for layeridx=1,opt.L_fc-2 do
    if opt.dropout>0 then fc:add(nn.Dropout(opt.dropout)) end
    fc:add(nn.Linear(opt.H_fc, opt.H_fc))
    --fc:add(nn.ReLU())
    fc:add(nn.LeakyReLU())
  end
  fc:add(nn.Linear(opt.H_fc, output_size))

  proj = fc(top_h)
  local logsoft = nn.LogSoftMax()(proj)

  table.insert(outputs, logsoft)

  final_rnn = nn.gModule(inputs, outputs)

  return final_rnn

end

return GRU_uni_multi_diagRNN

